...
const ParticipationStatus = z.enum([
  "ready",
  "not_eligible",
  "already_won",
  "already_lost",
...
export default async function handler(
  ...
  try {
    ...
    const amtAndProof = await ifEligibleGetProof(
      query.data.address,
      query.data.chain
    );
    if (!amtAndProof) {
    ...
    const participationStatusRes = await checkParticipationStatus(
      query.data.address,
      query.data.chain
    );
    return res.status(200).json({
...
const AirdropRegistry = {
  ...
  stargaze: {
    ...
    contract:
      "stars1qytke02t6mrj6zvyjymrd4e6h3l78ecx2nwwf9huvcprehy7xmsqpqh7za",
  },
  aura: {
...
const checkParticipationStatus = async (
  ...
  const res = await batchClient.wasm.queryContractSmart(entry.contract, {
    participant: {
      address: addr,
    },
  });
  ...
  if (
    res.participant.is_winner === null ||
    res.participant.is_winner === undefined
  ) {
    return {
